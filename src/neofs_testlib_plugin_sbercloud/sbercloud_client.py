...
class SbercloudClient:
    ...
    def find_volume_id_by_pci_address(self, node_id: str, pci_address: str) -> str:
        ...
        if "volumeAttachments" not in response_json:
            raise KeyError(f"volumeAttachments not found in response: {response_json}")
        ...
        if not queried_volume:
            raise KeyError(f"Device with {pci_address} not found in volumes list: {volumes}")

        if "volumeId" not in queried_volume[0]:
            raise KeyError(f"Device with {pci_address} have no volume id: {queried_volume}")
    ...
    def get_ecs_node_id(self, ip: str) -> str:
        ...
        desired_server = [server for server in response["servers"] if self._has_key_value(server, "addr", ip)]

        if not desired_server:
            raise Exception(f"Cannot find server with ip {ip}: {response}")
    ...
    def _has_key_value(self, obj: Union[list, dict], key: str, value: str):
        if isinstance(obj, list):
            for item in obj:
                return self._has_key_value(item, key, value)
        elif isinstance(obj, dict):
            for k, v in obj.items():
                if k == key and v == value:
                    return True
                if isinstance(v, (dict, list)):
                    return self._has_key_value(v, key, value)
